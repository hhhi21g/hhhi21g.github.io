...
class Solution {
    ...
    List<Integer> pathStack = new ArrayList<>(); // 模拟路径栈
    ...
    public boolean canFinish(int numCourses, int[][] prerequisites) {
        ...
        // 打印所有在环上的节点
        if (hasCycle) {
            System.out.println("图中存在环，环中节点有：" + cycleNodes);
        }
    ...
    void traverse(List<Integer>[] graph, int s) {
        if (onPath[s]) {
            // 检测到环：从 pathStack 中找出环路径
            hasCycle = true;

            int idx = pathStack.lastIndexOf(s);  // 找到环开始的位置
            for (int i = idx; i < pathStack.size(); i++) {
                cycleNodes.add(pathStack.get(i));
            }
            cycleNodes.add(s); // 当前节点也在环中
            return;
        }
